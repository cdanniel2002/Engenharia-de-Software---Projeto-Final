...
# WeasyPrint precisa do GTK3 Runtime no Windows. Instalamos via winget e
# adicionamos o bin do GTK3 ao PATH dos subprocessos para o import funcionar
# sem precisar reiniciar o terminal.
...
def ensure_gtk3():
    ...
    # PATH só é atualizado para processos novos; injeta no ambiente atual para
    # que os subprocess.Popen vejam o GTK3 sem precisar reiniciar o shell.
...
def install_python_deps():
    ...
    # psycopg[c] precisa do pg_config (PostgreSQL dev headers + compilador C).
    # No Windows é uma dor de cabeça compilar, e em dev usa-se SQLite por padrão
    # (USE_POSTGRESQL=False), então trocamos pelo wheel binário que já vem pronto.
...
def start_servers():
    # Garante o GTK3 no PATH mesmo quando o usuário só roda --run.
    if IS_WINDOWS:
        gtk_bin = find_gtk3_bin()
